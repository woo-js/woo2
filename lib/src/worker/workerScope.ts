...
export const SymObjectProp = Symbol('SymObjectObserver');
...
export class WorkerScope {
  ...
  private _makeObjectPropGetSet(obj: any, prop: string) {
    ...
    // 获取当前属性的get,set描述
    let oldGetter = desc?.get;
    let oldSetter = desc?.set;
    ...
    Reflect.defineProperty(obj, prop, {
      get() {
        // 跟踪属性调用
        if (_globalTraceKey) {
          log.info('addPropDependent', obj, prop, `to: ${_globalTraceKey}`);
          // 添加属性依赖
          dependents.addPropDependent(_globalTraceKey, prop);
          // 如果值为对象,则添加为值对象的自身依赖
          if (typeof desc.value === 'object') {
            if (Reflect.has(desc.value, SymObjectObserver)) {
              log.info('addSelfDependent', desc.value, `to: ${_globalTraceKey}`);

              let valueDependents = desc.value[SymObjectObserver];
              valueDependents.addSelfDependent(_globalTraceKey);
            }
          }
        }

        return oldGetter ? oldGetter() : desc.value;
      },
      set(value) {
        // 添加到当前属性变更通知
        let propDeps = dependents.getPropDependents(prop);
        log.info('set dependents', prop, propDeps?.size, propDeps);

        if (propDeps && propDeps.size > 0) {
          _globalScopeNotifier.addNoticeSet(_this._scopeName, propDeps);
        }
  ...
  private _makeObserver(obj: any): any {
    ...
    // console.info('makeObserver', obj);

    // 如果是数组,则创建数组观察对象
    if (obj instanceof Array) {
      return this._makeArrayObserver(obj);
    }
        ...
        new Proxy(newProto, {
          ...
          set(target, prop, value, receiver) {
            ...
            function _deepNoticeObj(obj: any) {
              ...
              // 通知对象自身的依赖
              let selfDeps = objDependents.getSelfDependents();
              if (selfDeps.size > 0) {
                _globalScopeNotifier.addNoticeSet(_this._scopeName, selfDeps);
              }
              Reflect.ownKeys(obj).forEach((k) => {
                if (typeof k !== 'string') return;
                // 通知对象的所有属性发生变更
                let propDeps = objDependents.getPropDependents(k);
                if (propDeps && propDeps.size > 0) {
                  _globalScopeNotifier.addNoticeSet(_this._scopeName, propDeps);
                }
          ...
          deleteProperty(target, p) {
            ...
            delete obj[p];
            // 添加当前对象的变更通知
            _globalScopeNotifier.addNoticeSet(_this._scopeName, dependents.getSelfDependents());
  ...
  private _makeArrayObserver(arr: any[]) {
